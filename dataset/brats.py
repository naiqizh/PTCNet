...
class Brats(Dataset):
    def __init__(self, patients_dir, benchmarking=False, training=True, data_aug=False,
        ...
        self.patterns = ["-t1n", "-t1c", "-t2w", "-t2f"] #2023
        #self.patterns = ["_t1", "_t1ce", "_t2", "_flair"] #2021
        if not no_seg:
            print("have_seg")
            self.patterns += ["-seg"] #2023
            #self.patterns += ["_seg"] #2021
    ...
    def __getitem__(self, idx):
        ...
        # print("_patient_id:",_patient['id'])
        # print("_patient_t1:",_patient['t1'])
        ...
        if _patient["seg"] is not None:
            #et = patient_label == 4 #2021
            et = patient_label == 3 #2023
            et_present = 1 if np.sum(et) >= 1 else 0
            #tc = np.logical_or(patient_label == 4, patient_label == 1)  #2021
            tc = np.logical_or(patient_label == 3, patient_label == 1)   #2023
...
def get_datasets(seed, on="train", fold_number=0, normalisation="minmax"):
    ...
    print("train_base_folder:",base_folder)
...
def get_test_datasets(seed, on="train", fold_number=0, normalisation="minmax"):
    ...
    print("test_base_folder:",base_folder)
    assert base_folder.exists()
    patients_dir = sorted([x for x in base_folder.iterdir() if x.is_dir()])
